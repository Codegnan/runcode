...
def execute_testcase(request, timeout, testcase):
    ...
    p = sp.Popen([sys.executable, '-c', cmd, '--existing'],
        #stdout=PIPE, stderr=PIPE, close_fds=True,
        stdout=sp.PIPE, stderr=sp.STDOUT, close_fds=(sys.platform != 'win32'),
    )
...
def run_code(request, qid):
    ...
    # Create a temporary Python file to store submitted program
    with open(os.path.join(BASE_DIR, '_auth_user_hash.py'), 'w') as fo:
        for ch in snippet:
            fo.write(ch)
